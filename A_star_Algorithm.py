...
# --- Find a free point (for start or goal) ---
...
# --- Plot grid view with path ---
def visualize_grid(grid, path=None, start=None, goal=None):
    color_grid = np.stack([grid * 255] * 3, axis=-1).astype(np.uint8)  # gray view
    color_grid[grid == 1] = [0, 0, 0]  # obstacles (black)
    color_grid[grid == 0] = [255, 255, 255]  # free (white)

    if path:
        for r, c in path:
            color_grid[r, c] = [0, 0, 255]  # path = red
    if start:
        color_grid[start[0], start[1]] = [0, 255, 0]  # start = green
    if goal:
        color_grid[goal[0], goal[1]] = [255, 0, 0]  # goal = blue
...
start = find_free_spot(grid, 'top-left')
goal = find_free_spot(grid, 'bottom-right')
